...
#
# 使い方:
#   ./scripts/install-cron.sh            # 既定スケジュールで登録（日次3:00 / 週次 日曜4:00 / 月次 毎月1日7:00）
#   DAILY_CRON="30 2 * * *" WEEKLY_CRON="0 5 * * 0" MONTHLY_CRON="0 8 1 * *" ./scripts/install-cron.sh  # 時刻変更
#
...
MONTHLY_TIME="${MONTHLY_CRON:-0 7 1 * *}"
