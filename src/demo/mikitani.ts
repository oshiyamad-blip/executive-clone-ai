...
export const MIKITANI_PROFILE: ExecutiveProfile = {
  ...
  // 採用基準（グローバル前提・プロフェッショナリズム・実行力を重視）
  hiringCriteria: [
    'グローバルに戦えるか（語学だけでなく多様な環境で成果を出せるか）',
    'スピードと実行力 — 考えるだけでなく自分で動いて結果を出したか',
    'プロフェッショナリズム — 結果に責任を持ち、KPIで語れるか',
    '仮説を立てて検証・改善を回した具体的なエピソードがあるか',
    '大胆なビジョンに共感し、長期で挑戦し続けられるか',
    '毎日1%改善を厭わない学習意欲・向上心があるか',
    '顧客価値・顧客満足を起点に考えられるか',
  ],
};
