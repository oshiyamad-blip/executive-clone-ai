import '../env.js';
import { notion, throttle } from '../database/index.js';
import { DB_IDS } from './notionDb.js';

// 案件・請求管理の Notion DB 一括作成スクリプト（npm run engagements:setup）
//
// NOTION_ENGAGEMENTS_PARENT_PAGE_ID 配下に6つのDBを作成し、
...
async function main(): Promise<void> {
  ...
  if (DB_IDS.project) {
    ...
    const db = await createDb(parentPageId, '案件DB', {
      ...
      ステータス: select('提案中', '募集中', '進行中', '終了', '失注'),
      期間開始: { date: {} },
      期間終了: { date: {} },
