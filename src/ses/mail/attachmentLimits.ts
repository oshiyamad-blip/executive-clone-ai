...
import { SPREADSHEET_MAX_BYTES } from '../spreadsheetText.js';
...
// 1通で保持する添付の件数の上限（PDF・それ以外）。小さな添付を何百件も付けたメール1通で、解析・抽出の時間を使い切らせない
export const MAIL_MAX_PDFS = 10;
export const MAIL_MAX_OTHER_ATTACHMENTS = 5;
...
// 保持する添付（純関数）。種類ごとの大きさ・件数の上限と1通の合計の上限の内に収まるものだけを先頭から選ぶ
export function attachmentsWithinLimits<T extends SizedAttachment>(items: T[]): { kept: T[]; dropped: number } {
  ...
  for (const a of items) {
    const pdf = isPdf(a);
    const cap = pdf ? PDF_MAX_BYTES : SPREADSHEET_MAX_BYTES;
    if (!(a.bytes >= 0) || a.bytes > cap || total + a.bytes > MAIL_ATTACHMENT_TOTAL_MAX_BYTES) continue;
    if (pdf ? pdfs >= MAIL_MAX_PDFS : others >= MAIL_MAX_OTHER_ATTACHMENTS) continue;
    if (pdf) pdfs += 1;
    else others += 1;
    total += a.bytes;
    kept.push(a);
