...
const NAME_LABEL = /(氏名|お名前|名前|本名|フルネーム)(\s*[:：]\s*)([^\n\r]*)/g;
...
// 敬称の直前の漢字2〜4文字（「田中太郎様」「鈴木さん」「田中 太郎 様」）。前後に漢字が続く語の一部は拾わない
const HONORIFIC_NAME = new RegExp(
  `(?<!${KANJI})(${KANJI}{1,3}[ \\u3000]${KANJI}{1,3}|${KANJI}{2,4})(?=\\s?(?:様|さま|さん|氏|殿|君))`,
  'g',
);
...
const NOT_A_NAME_SUFFIX = /[者社員位客部課係室局庁省店所会団組班陣達等方]$/;
...
function maskHonorificName(match: string): string {
  ...
  return nameLike(match) ? NAME_MASK : match;
}
...
const BARE_INITIALS = /^[A-Za-z]{2,3}$/;
...
function initialsFrom(part: string): string | null {
  const s = part.replace(HONORIFIC_SUFFIX, '').trim();
  if (!s) return null;
