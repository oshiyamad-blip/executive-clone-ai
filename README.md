...
npm run hire -- --file 候補者.txt  # 採用判断支援（合否の傾き＋評価点＋懸念＋深掘り質問）
npm run chat                 # CLIで経営者クローンと対話（壁打ち）
npm run web                  # ブラウザ対話UI（即断/採用/壁打ちトグル・http://127.0.0.1:8787）
...
| **採用判断支援** | `npm run hire -- --file 候補者.txt` | 候補者情報を社長の採用基準に照らして評価（合否の傾き＋評価点＋懸念点＋深掘り質問＋次アクション） | ターミナル / Web「採用」タブ |
| **CLI対話** | `npm run chat` | 手元での壁打ち | ターミナル |
| **Web対話UI** | `npm run web` | ブラウザで即断/採用/壁打ち（モード切替） | ブラウザ（参照元はNotionリンク） |
...
**採用判断** は候補者情報をそのまま渡せます（`npm run hire -- "職歴や面接メモ…"` または `--file <path>`）。
面接メモ・履歴書テキスト・面接の音声書き起こし（ライフログ）などを貼り付ければOKです。学歴/性別/年齢等の属性ではなく、
能力・行動・価値観（採用基準への適合）で評価するようにプロンプトで制約しています。**最終判断は必ず人が行う前提**の支援ツールです。
Web UIは `WEB_ACCESS_TOKEN`（32文字以上）を設定するとアクセス制御が有効になり、既定でローカル（127.0.0.1）にのみ待受します（要件4.1）。ローカル以外で待ち受けるには HTTPS（`WEB_TLS_CERT`/`WEB_TLS_KEY` か `WEB_BEHIND_TLS=true`）も必要です。
