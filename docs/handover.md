...
- **Mac**: [nodejs.org](https://nodejs.org/) から **LTS**（v20以上）のインストーラを実行（Homebrew があれば `brew install node` でも可）
- **Windows**: [nodejs.org](https://nodejs.org/) から LTS のインストーラを実行
- 確認: ターミナル（Windows は PowerShell）で `node --version` → `v20` 以上
