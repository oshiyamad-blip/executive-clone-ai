...
- `SES_DRAFT_SIGNING_KEY`（ランダムな32文字以上）を設定すると、「下書きデータ」列に署名を付け、シート上で書き換えられた行からは作りません
- `作成中 日時` のまま残った行は、作成中に実行が止まったか、作成直後の状態書き戻しに失敗したものです（二重作成を避けるため自動では
  再作成しません）。次のバッチが下書きフォルダで同じ識別子（X-SES-Draft-Key ヘッダ）の下書きを見つければ `作成済` にします。
