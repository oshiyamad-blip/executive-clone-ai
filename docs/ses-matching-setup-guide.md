...
- 案件の必須・尚可スキルは「要件」の単位で数えます。選択肢（「Java または C#」「AWS・GCP等」「AWS/GCP/Azureのいずれか」）と、
  括弧で例を挙げた記載（「AWS(EC2/RDS/Lambda)」「RDB(Oracle/MySQL)」）は、どれか1つを満たせば1要件を満たします
  （スプレッドシートには `Java / C#（いずれか）` `AWS(EC2/RDS/Lambda)` の形で保存し、読み戻しても同じ要件になります）。
