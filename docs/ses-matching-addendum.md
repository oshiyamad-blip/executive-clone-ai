...
例: 同単金（粗利0）のペアを **案件の値上げ・要員の値下げ** で交渉し、粗利下限（設定値）を確保する。

**ロジック**（`src/ses/match.ts`）:
- 粗利が下限未満のペアについて、不足分を「案件単金の値上げ」と「要員単金の値下げ」で埋められるか判定。
- 不足分をできるだけ**両者で折半**し、各交渉上限（設定値）で頭打ち。切り上げで下限を確実に満たす。
- 交渉幅（値上げ上限＋値下げ上限。設定値）を**超える場合のみ除外**。それ以内なら「交渉提案」枠として拾う。
...
| サマリ（`notify.ts`） | 【交渉提案】セクションを追加（「案件+◯万／要員−◯万で粗利◯万円」形式） |
...
## 10. データ保存先の切替（Notion ⇄ Googleスプレッドシート）

**背景**: 「Notionのかわりに GWS（スプレッドシート）で代用したい」という要望に対応。
Notionのチーム課金回避と、営業のスプシ文化（ソート・フィルタ・目視運用）との親和性が狙い。

- `MAIL_PROVIDER`／`LLM_PROVIDER` と同じ流儀で **`DB_PROVIDER=notion（既定）| sheets`** を追加
- `src/database/index.ts` を唯一の窓口（ファサード）として維持し、SES系12関数の先頭で
  Sheets実装（`src/database/sheets.ts`）へ委譲。**マッチング・UI・メール処理は無変更**
- ラベル変換・返信メタ・営業元の結合/分解は `src/database/mapping.ts` に集約し、
  両バックエンドが**同じ表記**で保存・復元する（後からのデータ移行も容易）
- Sheets側の特徴:
  - 1スプレッドシート6タブ（案件/要員/マッチ/自社社員/評価/スキル同義）。**タブ・ヘッダーは初回自動生成**
  - 案件・要員はID、マッチはタイトルでupsert（再実行で行が増殖しない。人が変えたステータスは保持）
  - 書き込みクォータ（60回/分/ユーザー）対策で全呼び出しを1.1秒間隔に直列化
  - スキル読出時の正規化・返信メタ（全員に返信）の永続化などNotion版の修正をすべて踏襲
- 必要な準備: 空シート作成＋`SHEETS_DB_SPREADSHEET_ID`設定＋DWDに `spreadsheets` スコープ追加（導入マニュアル4-7）
- 設定不足時は warn して縮退（Notion版と同じ）。demoは従来どおりDBに一切触れない

## 11. プロパー（自社社員）のスキルシート → 案件候補

**背景**: 自社社員のスキルシートはDriveのフォルダで管理されており、人が「自社社員」タブへ転記する運用は続かない。
スキルシートを置くだけで社員の情報が揃い、人は採算ライン（必要案件単価）と稼働状況だけを付ければ候補が出る形にした。

- 構成（`src/ses/proper/`）: `drive.ts`（フォルダ一覧・PDF/Excel/Word/Googleドキュメント/スプレッドシートの読み取り）→
  `extractSkillSheet.ts`（抽出用モデル＋構造化出力で1ファイル1コール）→ `master.ts`（管理表「プロパー管理」の同期と読み出し）→
  `index.ts`（`ownMatch.ts` の閾値方式で突合し、案件スプシの「プロパー候補」タブへ保存）・`proposal.ts`（提案文面の定型文）
- 管理表は1ファイル1行。人の列（氏名・提案用表記・稼働状況・必要案件単価・稼働可能日）は入力済みなら決して上書きしない。
  変更のないファイルはLLMを呼ばず、1回の抽出件数に上限（`PROPER_MAX_EXTRACT_PER_RUN`）を設けて初回の大量取り込みを分割する
- 提案文面は社外に出るため、提案用表記（イニシャル）だけを使い、氏名・必要案件単価は書かない。LLMは使わない（定型文）。
  「担当者メール」列による下書き作成（導入マニュアル4-7）の対象に「プロパー候補」タブ（案件側のみ）を加えた
- 認証は既定でメインのサービスアカウント（同じWorkspace内で共有）。別テナント運用時のみ `PROPER_GOOGLE_SA_*`／`PROPER_GOOGLE_IMPERSONATE`
- Sheetsの汎用部分（タブ自動生成・ヘッダー移行・行キャッシュ・クォータ制御）は `src/database/sheetBook.ts` に切り出し、
  案件スプシと管理表がそれぞれインスタンスを持つ
...
- `npm run ses:demo` … 成立候補1件・**交渉提案1件**（金融系Java案件×M.T.: 案件60万/要員58万 → 案件・要員の単金を交渉すればデモの粗利下限に届く）・要確認1件を検出。交渉下書きには双方への単金相談を自動で反映
...
- 自社社員→案件の紹介メール下書きは、プロパー（スキルシート連携・第11節）の「プロパー候補」タブで提供（従来の「自社社員」タブの突合は候補の提示まで）。
