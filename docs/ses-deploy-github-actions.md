...
- 案件・要員タブの「指示混入疑い」列が `あり` の行は、元のメールに AI への指示らしき記載（「以前の指示を無視せよ」等）があったものです
  ...
  この印は署名した「返信メタ」の中にも残すため、列の `あり` を消しても要確認のままです
...
4. **次の回（10:00 または 14:00）の最初に下書きが作られ**、状態が `作成済 日時` になります
   ...
   - 状態が `作成中 日時 #記号` のまま残っている行は、前の回が途中で止まった可能性があります（`#` の後ろはバッチが照合に使う記号です）。下書きフォルダを確認し、
...
- **【運用者の作業】スプレッドシートの編集者は必要な営業だけにし**、「下書きデータ」列・「_状態」タブは **データ → シートと範囲を保護** で
  運用担当とサービスアカウントだけが編集できるようにしてください（編集者は宛先・文面の元になる列や、隔離リストを書き換えられます）
...
- 社外から届くメールは、1通45MB超は取り込まず（サマリに件数だけ載せます。受信箱で直接確認してください）、使えない大きさの添付
  ...
  件名は500文字までにします
- 同時に2つのバッチ（定時実行と手元の `npm run ses`・自前の定期実行など）は動きません（2つ目は何もせずに異常終了します）
