# SESマッチング 本番導入手順書（GitHub Actions で平日10:00〜19:00に毎時自動実行）
...
```
 GitHub Actions（平日 10:00〜19:00 に毎時自動で起動。毎回まっさらな環境で動いて終わる）
   │
...
5. **main を保護する（必須）**: **Settings → Rules → Rulesets → New branch ruleset** で、対象を既定のブランチ（main）にし、
   ...
   **2つのワークフロー（定時バッチ・メール量の測定）は、鍵の無いビルドのジョブで GitHub API から main のルールを読み、上の規則
   （PR・承認 1 以上・最後の push 以外の承認・古い承認の取り消し・Code Owners・force-push と削除の禁止）が1つでも欠けていると
   「main のルールセットを確認」のステップで失敗し、鍵を持つジョブを動かしません**（3章の測定の前に設定してください）。
   Bypass list が空かどうかは API から確かめられないため、画面で確認してください。
   **この厳しい設定には、main に書き込める人（コードオーナー）が2人以上必要です**（GitHub では自分の PR を自分で承認できないため、
   1人だけでは修正の PR を取り込めなくなります）。**書き込めるのがオーナー1人だけのリポジトリ**では、ルールセットの
...
1. このリポジトリの main ブランチに `.github/workflows/ses-mail-stats.yml` が入っていることを確認します
   ...
   **先に 2-4 の 5 の main のルールセットを設定してください**（未設定だと「main のルールセットを確認」で止まり、測定は動きません）
...
5. **案件だけモード（既定 `SES_TARGET=projects`）**では、パートナーから届く要員メールは抽出しません。売りたい要員は「プロパー管理」タブに登録します。
   ...
   - 手で追加した行には、次の回に `手動ID` が自動で入ります（消さない・書き換えないでください）。
   - パートナーの要員は `区分` に「パートナー」と入れます（空欄は自社社員。提案文面の「弊社社員」の表記が変わります）。
...
6. **要員データの保存期間**：要員管理表の行は自動では消しません。人が「稼働状況」を「アサイン済」「対象外」にするか、行を削除するまで、そのまま案件探しに使います。
...
| `XSERVER_AUTHSERV_ID` | `sv1234.xserver.jp` | 推奨 | 受信サーバーが付ける Authentication-Results の最初の名前（authserv-id。1つだけ・完全一致。ワイルドカード不可）。受信したメールの原文（「メールのソース」）で、**一番上の** Authentication-Results が この名前で始まり `dmarc=` を含むことを確かめてから登録します（同じサーバーのアカウントから送ったテストメールのヘッダも確かめます。9章の【メール】の項）。未設定・一致しないメールは送り主の認証結果を使いません（ヘッダは送り主も書けるため）。設定しても一致するメールが無い回はサマリで知らせます |
...
main に入った時点から平日 10:00〜19:00 の毎時に動き始めます。Secrets の登録が終わるまでは、
...
以後は平日の 10:00〜19:00 に毎時自動で動きます（GitHub の混雑で数分〜数十分遅れることがあります。祝日も動きます）。
...
1. **サマリメールを見る**（新しい候補が見つかった回だけ届きます。毎回受け取りたい場合は Variable `SES_NOTIFY_ALWAYS=true`）。成立候補・交渉提案・参考提案・要確認の件数と一覧が載っています
...
| main のルールセットを確認 | ❌ の下に足りない規則が出ます。2-4 の 5 のとおりルールセットを直して「Re-run jobs」（API に届かないときも止まるので、その場合は時間をおいて再実行） |
...
- **main への変更はレビュー必須（2-4 の 5 のルールセット）**: 鍵は main の実行に渡るため、main に直接 push できる人・トークン・自動化があると、
  ...
  「任意」ではなく**本番を始める前に必ず**設定してください（未設定・不足なら、鍵の無いビルドのジョブが鍵を持つジョブを止めます。
  ただしこの確認は main のコードにあるため、すでに main に push できる人には外せます。ルールセットそのものが守りです）
- **メール量の測定も、鍵を持つジョブでビルドしません**: `ses-mail-stats.yml` も `ses-batch.yml` と同じく、ビルドは鍵の無いジョブで行い、
  鍵を持つジョブは別の環境で本番用の依存パッケージだけを入れて、受け取った `dist/` を動かします
...
- **【メール】受信サーバーの送信ドメイン認証**: 再送スキップは、受信サーバーが付けた一番上の `Authentication-Results` で
  ...
  そのサーバーのもので、結果より上に他のサーバーの `Received` が無く、さらにメールが受信サーバーに入った段の `Received` が外のサーバーからの
  SMTP の受け取り（`with ESMTP`・`ESMTPS`。認証して送った `ESMTPA`・`ESMTPSA`・`Authenticated sender` や、サーバーの中から出した
  `from` の無い段でない）ときだけ信じます（同じ共有サーバーを借りた第三者が、そのサーバーから送ったメールに自分で結果を書けるため）。
  登録の前に、社外からのメールに加えて、**同じサーバーのメールアカウントから共有メールボックスへ送ったテストメール**のヘッダも確かめ、
  そのメールの `Received` に `ESMTPSA` 等の認証した送信の段があること（その経路の結果は信じません）を確認してください。サマリに「受信サーバーを通ったのに信じてよい認証結果が
