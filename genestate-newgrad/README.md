...
| 形 | A 片角スイープ ／ B 四隅まるく ／ C 直線 ／ **D 斜めカット** ／ **E アーチ** |
| **各パートの構成** | 「構成を変える」を開くと、11か所をそれぞれ A / B で切り替えられる |
| 表示 | PC ／ スマホ |

切替バーは右端の「引っ込める」（または `Esc` キー）で丸ごと隠せる。
意匠だけを素の状態で見たいときに使う。隠すと右下に小さな戻し口が出る。
PC表示・スマホ表示のどちらでも同じように効き、隠したことも端末に記憶される。

**実機のスマートフォンで開いた場合**は、バーだけで画面の半分近くを占めてしまうため、
最初から引っ込めた状態で開く（右下の戻し口から出せる。一度自分で開閉すれば以降はその選択が優先される）。
あわせて狭い画面ではバーの部品を折り返し、端末の枠（375px固定）も外して幅いっぱいに使う。
...
// 構成は変えたいパートだけ書けばよい
define( 'NG_COMP_PEOPLE',  'b' );   // 人だけ横スクロールに
define( 'NG_COMP_RECRUIT', 'b' );   // 募集要項だけカードに
```
...
**形**は色や書体と独立した軸で5案。角の形はこれまでトンマナに紐づいていたが、
「色は A のまま形だけ変えたい」が選べなかったため切り出した。
**各パートの構成**はさらに独立した軸で、11か所に2案ずつある
（イントロ／代表メッセージ／数字／事業／仕事／育成／人／はたらく環境／募集要項／FAQ／エントリー）。
どちらが正しいという話ではなく、**載せる分量と写真の枚数**で決まるため、
色や形が固まってから見比べるとよい。定数は `NG_COMP_*`（数字と育成だけ
先に入れた `NG_NUMBERS` / `NG_GROWTH` を使う）。
